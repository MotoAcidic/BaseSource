...
void CObfuscationPool::ProcessMessageObfuscation(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "dsa") { //Obfuscation Accept Into Pool
        ...
        BOOST_FOREACH (const CTxIn item, sigs) {
            if (AddScriptSig(item)) svkcess = true;
...
//
// Unlock coins after Obfuscation fails or svkceeds
//
...
std::string CObfuscationPool::GetStatus()
{
    ...
    if (chainActive.Tip()->nHeight - cachedLastSvkcess < minBlockSpacing || !masternodeSync.IsBlockchainSynced()) {
...
void CObfuscationPool::Check()
{
    ...
    if ((state == POOL_STATUS_ERROR || state == POOL_STATUS_SVKCESS) && GetTimeMillis() - lastTimeChanged >= 10000) {
...
void CObfuscationPool::CheckFinalTransaction()
{
    ...
    LOCK2(cs_main, pwalletMain->cs_wallet);
    {
        ...
        RelayCompletedTransaction(sessionID, false, MSG_SVKCESS);
    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat VKC and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 VKC per transaction on average.
            */
            if (r <= 10) {
...
void CObfuscationPool::CheckTimeout()
{
    ...
    if (!fMasterNode) {
        ...
        case POOL_STATUS_ERROR:
            ...
            break;
        case POOL_STATUS_SVKCESS:
            LogPrint("obfuscation", "CObfuscationPool::CheckTimeout() -- Pool svkcess -- Running Check()\n");
            Check();
            break;
        }
    }
...
{
    if (fMasterNode) return false;
    if (state == POOL_STATUS_ERROR || state == POOL_STATUS_SVKCESS) return false;
...
void CObfuscationPool::CompletedTransaction(bool error, int errorID)
{
    ...
    if (error) {
        ...
        LogPrintf("CompletedTransaction -- svkcess \n");
        UpdateState(POOL_STATUS_SVKCESS);

        UnlockCoins();
        SetNull();

        // To avoid race conditions, we'll only let DS run once per block
        cachedLastSvkcess = chainActive.Tip()->nHeight;
    }
    lastMessage = GetMessageByID(errorID);
}
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    if (chainActive.Tip()->nHeight - cachedLastSvkcess < minBlockSpacing) {
...
bool CObfuscationPool::SendRandomPaymentToSelf()
{
    ...
    bool svkcess = pwalletMain->CreateTransaction(vecSend, wtx, reservekey, nFeeRet, strFail, coinControl, ONLY_DENOMINATED);
    if (!svkcess) {
    ...
    LogPrintf("SendRandomPaymentToSelf Svkcess: tx %s\n", wtx.GetHash().GetHex());
...
bool CObfuscationPool::MakeCollateralAmounts()
{
    ...
    bool svkcess = pwalletMain->CreateTransaction(vecSend, wtx, reservekeyChange,
    ...
    if (!svkcess) {
        ...
        svkcess = pwalletMain->CreateTransaction(vecSend, wtx, reservekeyChange,
    ...
    // use the same cachedLastSvkcess as for DS mixinx to prevent race
...
void CObfuscationPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100VKC+1 ( bit on if present )
    // bit 1 - 10VKC+1
    // bit 2 - 1VKC+1
    // bit 3 - .1VKC+1
    // bit 3 - non-denom
