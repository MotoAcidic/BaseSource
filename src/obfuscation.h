...
#define POOL_STATUS_SVKCESS 8              // svkcess
...
{
public:
    /// Is the inputs associated with this public key? (and there is deposit VKC amount - checking if valid masternode)
    ...
    /// Verify the message, returns true if svkccessful
...
public:
    ...
    {
        if (fMasterNode && (newState == POOL_STATUS_ERROR || newState == POOL_STATUS_SVKCESS)) {
            LogPrint("obfuscation", "CObfuscationPool::UpdateState() - Can't set state to ERROR or SVKCESS as a Masternode. \n");
            return;
        }
    ...
    /// Get the denominations for a specific amount of VKC.
