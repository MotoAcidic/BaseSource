...
bool CAddrInfo::IsTerrible(int64_t nNow) const
{
    ...
    if (nLastSvkcess == 0 && nAttempts >= ADDRMAN_RETRIES) // tried N times and never a svkcess
        return true;

    if (nNow - nLastSvkcess > ADDRMAN_MIN_FAIL_DAYS * 24 * 60 * 60 && nAttempts >= ADDRMAN_MAX_FAILURES) // N svkcessive failures in the last week
        return true;
